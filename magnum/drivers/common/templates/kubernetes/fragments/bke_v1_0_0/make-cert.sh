...
EOF
    ...
    curl $VERIFY_CA -X GET \
        ...
        $MAGNUM_URL/certificates/$CLUSTER_UUID"?ca_cert_type="${_CA_CERT_TYPE} | python3 -c 'import sys, json; print(json.load(sys.stdin)["pem"])' > ${CA_CERT}
    ...
    csr_req=$(python3 -c "import json; fp = open('${_CSR}'); print(json.dumps({'ca_cert_type': '$_CA_CERT_TYPE', 'cluster_uuid': '$CLUSTER_UUID', 'csr': fp.read()})); fp.close()")
    curl $VERIFY_CA -X POST \
        -H "X-Auth-Token: $USER_TOKEN" \
        -H "OpenStack-API-Version: container-infra latest" \
        -H "Content-Type: application/json" \
        -d "$csr_req" \
        $MAGNUM_URL/certificates | python3 -c 'import sys, json; print(json.load(sys.stdin)["pem"])' > ${_CERT}
}
