...
EOF
    ...
    curl $VERIFY_CA -X GET \
        ...
        $MAGNUM_URL/certificates/$CLUSTER_UUID | python3 -c 'import sys, json; print(json.load(sys.stdin)["pem"])' >> $CA_CERT
    ...
    csr_req=$(python3 -c "import json; fp = open('${_CSR}'); print(json.dumps({'cluster_uuid': '$CLUSTER_UUID', 'csr': fp.read()})); fp.close()")
